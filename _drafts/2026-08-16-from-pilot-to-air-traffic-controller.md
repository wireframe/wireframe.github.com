...
Reviewing a change used to require hallway conversations and architecture reviews to recover the context, the intent, and the phasing. These artifacts capture all of it up front, so an agent and a human reviewer can both see the assumptions behind a specific set of changes, and argue with them before anything gets built.
