...
**Geography is a worse filter than your hiring bar.** I've tried to build engineering teams in geo-constrained markets, and the math gets brutal fast. [PERSONAL ANECDOTE — a specific moment where the local pool ran out, the role you couldn't fill, the offer that got outbid, etc.] Every strong engineer is already employed, comp is punishing, and the same fifteen candidates are interviewing at the same fifteen companies. Remote-first lets you fish the global pond instead of the thirty-mile one, and the candidate pool grows by roughly an order of magnitude. The practical limit isn't geography anymore, it's timezone overlap: flexible hours are a perk, core hours are a requirement. Screening gets harder, not easier, since you're now competing globally for the top of every market.
...
**Supporting early-career engineers is harder remote, and it changes how you build the team.** In an office, juniors learn by osmosis: overhearing a debate, watching a senior debug, leaning over to ask the person next to them. Remote kills that channel. Mentorship has to be deliberately scheduled, and each junior costs more senior hours than it would in person. In my own experience, the senior-to-junior ratio that worked at 1:5 in-office had to stretch to about 1:10 remote. There just wasn't enough senior bandwidth to support more juniors at the old ratio. Brooks named the underlying mechanism in 1975 (*The Mythical Man-Month*): adding people diverts existing engineers to educate them, and communication paths grow faster than headcount. Netflix took the extreme path, hiring only senior engineers for ~25 years (≈0% junior vs. 30–50% at peers) and reversing it in 2023. The senior-only model has real limits.
